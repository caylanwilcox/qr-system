...
const ManageAdmins = () => {
  ...
  const fetchAdmins = async () => {
    ...
    try {
      ...
      if (snapshot.exists()) {
        const usersData = snapshot.val();
        // Filter for admins or super-admins
        const adminsArray = Object.entries(usersData)
          .filter(([_, data]) => data.role === 'admin' || data.role === 'super-admin')
          .map(([id, data]) => ({
            id,
            ...data
          }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing && editingId) {
        ...
        // Overwrite the existing record with newAdmin data (forcing role: 'admin')
        await set(adminRef, { ...newAdmin, role: 'admin' });
        setSuccessMessage('Administrator updated successfully');
      } else {
        // Create a brand-new user node
        const newId = new Date().getTime(); // or push(ref(...)).key
        const adminRef = ref(database, `users/${newId}`);
  ...
  const handleChangeRole = async (userId, newRole) => {
    try {
      ...
      if (snapshot.exists()) {
        ...
        // Re-fetch admins to see if they're still an admin
        fetchAdmins();
        setTimeout(() => setSuccessMessage(''), 3000);
      }
    } catch (err) {
  ...
  return (
    <div className="dashboard-container p-6">
      ...
      <form onSubmit={handleSubmit} className="quadrant p-6 rounded-lg mb-8 bg-gray-800 bg-opacity-40">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="relative">
            <input
              ...
              required
              className="bg-opacity-20 bg-gray-800 border border-gray-700 rounded-lg p-3 text-white 
                         placeholder-gray-400 w-full focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
            {/* Suggestion Dropdown */}
          ...
          <select
            ...
            onChange={handleInputChange}
            required
            className="bg-opacity-20 bg-gray-800 border border-gray-700 rounded-lg p-3 text-white w-full 
        ...
        <button
          type="submit"
          className="mt-6 bg-blue-500 bg-opacity-20 hover:bg-opacity-30 text-blue-400 font-medium px-6 py-2 rounded-lg 
                     flex items-center gap-2 border border-blue-500 border-opacity-20 transition-all duration-300"
        >
          <PlusCircle size={20} />
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-700">
            ...
            <tbody className="divide-y divide-gray-700 bg-opacity-20">
              {admins.map((admin) => {
                // Example: If you store attendanceRate in admin.stats.attendanceRate
                // or anywhere else, adjust accordingly:
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex gap-2">
                        ...
                        {/* "Delete" button: demotes to 'employee' */}
                        <button
